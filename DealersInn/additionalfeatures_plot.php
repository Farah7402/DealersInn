<?php
	...
	if(isset($_POST['btnSubmit']))
	{
		...
		$Query = "SELECT feature_id, data_type FROM addition_features_plot 
		...
		if(mysqli_num_rows($rstRow) > 0)
		{
			while ($objRow = mysqli_fetch_object($rstRow)) 
			{
				...
				if($Value != "")
				{
					$Query = "INSERT INTO property_additional_features_plot(property_id, feature_id, feature) 
  	...
  	<div id="content">
  		<div class="container">
              ...
              <div class="col-md-7 col-lg-8 col-xl-8">
                ...
                <form action="additionalfeatures_plot.php" method="POST">
                <?php
                  $Query = "SELECT feature_id, feature, data_type FROM addition_features_plot WHERE property_type=".$_REQUEST["Type"];
                  $rstRow = mysqli_query($Conn, $Query);
                  if(mysqli_num_rows($rstRow) > 0)
                  {
                    while ($objRow = mysqli_fetch_object($rstRow)) 
                ...
                <?php
                  ...
                  {
                ?>
                  <input type="text" name= "input<?=$objRow->feature_id;?>" class="form-control form-control-lg" id="input<?=$objRow->feature_id;?>" placeholder="<?=$objRow->feature;?>">
                <?php
                  }
                  else
                  {
                    $Query = "SELECT option_id, optionvalue FROM addition_features_options_plot
                ...
                ?>
                  <div class="checkbox">
                    <input  name="input<?=$objRow->feature_id;?>[]" type="checkbox" id="input<?=$objRow->feature_id."-".$objPro->option_id;?>" value="<?=$objPro->optionvalue;?>">
              ...
              ?>
                ...
                <div class="form-group">
                  ...
                  <button type="submit" name="btnSubmit" value="SEND" class="btn btn-lg btn-primary">Submit Property</button>
                </div>
              </div>
            </form>
          </div>
